...
const Review = () => {
  ...
  return (
    <section className="customer-flow mx-auto mb-20 mt-10 max-w-xl animate-fade-in">
      <span className="section-kicker">Reseña verificada</span>
      ...
      <form onSubmit={submit} className="glass-card mt-8 space-y-5 rounded-[2rem] p-6 md:p-8">
        <div>
          ...
          <div className="flex gap-2 text-3xl">
            {[1, 2, 3, 4, 5].map((star) => (
              <button key={star} type="button" onClick={() => setRating(star)} className={star <= rating ? "text-[#d99b4f]" : "text-stone-200"}>★</button>
            ))}
          </div>
        </div>
        <input value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="Tu nombre" className="w-full rounded-xl px-4 py-3" />
        <textarea value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Comentario opcional" className="min-h-32 w-full rounded-xl px-4 py-3" />
